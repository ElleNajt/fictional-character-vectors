...
def process_roles_on_worker(
    worker_id: int, gpu_ids: List[int], role_names: List[str], args
):
    ...
    worker_logger.info(
        f"Starting processing on Worker {worker_id} with GPUs {gpu_ids} and {len(role_names)} roles"
    )
    ...
    try:
        ...
        for role_name, role_data in tqdm(
            role_files.items(), desc=f"Worker-{worker_id}", position=worker_id
        ):
            try:
                ...
                if responses:
                    ...
                    worker_logger.warning(
                        f"No responses generated for role '{role_name}'"
                    )
            except Exception as e:
                failed_count += 1
                worker_logger.error(f"Exception processing role {role_name}: {e}")
            finally:
                # Clear GPU cache after each role to prevent memory leaks
                gc.collect()
                torch.cuda.empty_cache()

        worker_logger.info(
            f"Worker {worker_id} completed: {completed_count} successful, {failed_count} failed"
        )
...
def run_multi_worker(args) -> int:
    ...
    if tensor_parallel_size > total_gpus:
        logger.error(
            f"tensor_parallel_size ({tensor_parallel_size}) cannot be greater than available GPUs ({total_gpus})"
        )
        return 1
...
def main():
    ...
    parser.add_argument(
        "--output_dir", type=str, required=True, help="Output directory for JSONL files"
    )
    parser.add_argument(
        "--max_model_len", type=int, default=2048, help="Maximum model context length"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--question_count", type=int, default=240, help="Number of questions per role"
    )
    parser.add_argument(
    ...
    tensor_parallel_size = (
        args.tensor_parallel_size if args.tensor_parallel_size else total_gpus
    )
    ...
    use_multi_worker = (
        ...
        and total_gpus > tensor_parallel_size
    )

    if use_multi_worker:
        logger.info(
            f"Multi-worker mode: {total_gpus} GPUs with tensor_parallel_size={tensor_parallel_size}"
        )
        ...
        generator.process_all_roles(skip_existing=True, roles=args.roles)
