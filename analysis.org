...
** Select Universe for Detailed View

#+begin_src python
# Configuration - change this to view different universes
DETAIL_UNIVERSE = 'harry_potter'  # Options: harry_potter, star_wars, marvel, game_of_thrones, 
                                  # lord_of_the_rings, naruto, greek_mythology, chinese_mythology,
                                  # hindu_mythology, norse_mythology, egyptian_mythology, shakespeare
#+end_src
...
** PC Details: Tables and Distributions

#+begin_src python
import matplotlib.pyplot as plt

def detailed_pc_analysis(universe_name, prefix, n_pcs=5, n_extreme=10):
    ...
    for pc_idx in range(min(n_pcs, u_pca.n_components_)):
        ...
        print("TOP 10 (positive direction):")
        rows = []
        for i in sorted_idx[-n_extreme:][::-1]:
...
if 'residuals_role' in dir():
    detailed_pc_analysis(DETAIL_UNIVERSE, UNIVERSES[DETAIL_UNIVERSE])
#+end_src
...
** All Universes Summary Table

Quick reference: Top 3 PCs for each universe with variance explained.
...
if 'residuals_role' in dir():
    ...
    for universe, prefix in UNIVERSES.items():
        ...
        rows.append({
            'Universe': universe.replace('_', ' ').title(),
            'N': len(indices),
            'PC1': f"{u_pca.explained_variance_ratio_[0]:.1%}",
            'PC2': f"{u_pca.explained_variance_ratio_[1]:.1%}",
            'PC3': f"{u_pca.explained_variance_ratio_[2]:.1%}",
            'Top 3': f"{cumvar[2]:.1%}",
            'Top 5': f"{cumvar[4]:.1%}",
        })
    
    df = pd.DataFrame(rows).sort_values('Top 5', ascending=False)
    print(df)
#+end_src
