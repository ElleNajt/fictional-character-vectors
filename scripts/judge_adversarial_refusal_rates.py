...
Output: results/adversarial_refusal_rates_judged.json
"""
import json
import os
import time
...
INPUT_PATH = REPO_ROOT / "results" / "adversarial_refusal_rates.json"
OUTPUT_PATH = REPO_ROOT / "results" / "adversarial_refusal_rates_judged.json"
...
def judge_response(question, response):
    ...
    for attempt in range(8):
        try:
            ...
            wait = 2 ** attempt
            print(f"    Empty/bad response, retrying in {wait}s...")
            time.sleep(wait)
