"""
Judge the villain refusal rate responses.

Usage:
    python scripts/judge_villain_refusal_rates.py [--resume]

Output: results/villain_refusal_rates_judged.json
"""
import json
import os
import time
...
INPUT_PATH = REPO_ROOT / "results" / "villain_refusal_rates.json"
OUTPUT_PATH = REPO_ROOT / "results" / "villain_refusal_rates_judged.json"
...
def main():
    ...
    for entry in data:
        ...
        if done % 10 == 0:
            print(f"[{done}/{total}] {entry['character']:15s} {entry['condition']:12s} run={entry['run']} {r}{m}")
